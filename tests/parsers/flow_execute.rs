...
fn it_parse_with_seq() {
    ...
    assert_eq!(('a', 'b'), r.execute(&"ab".as_bytes(), 0).fold(
...
fn it_parse_with_opt_success() {
    ...
    assert_eq!(Some('a'), r.execute(&"a".as_bytes(), 0).fold(
        |a, _, _| a.map(|a| a as char),
        |_, _| panic!("Parse error"),
    ));
}
...
fn it_parse_with_opt_success_empty() {
    ...
    assert_eq!(None, r.execute(&"".as_bytes(), 0).fold(
        |a, _, _| a,
        |_, _| panic!("Parse error"),
    ));
}
...
fn it_parse_with_optrep_success() {
    ...
    let s = 1024 * 1024;
    assert_eq!(s, r.execute(&"a".repeat(s).as_bytes(), 0).fold(
        |a, _, _| a.len(),
        |_, _| panic!("Parse error"),
    ));
}
...
fn it_parse_with_optrep_success_empty() {
    ...
    assert_eq!(0, r.execute(&"".as_bytes(), 0).fold(
        |a, _, _| a.len(),
        |_, _| panic!("Parse error"),
    ));
}
